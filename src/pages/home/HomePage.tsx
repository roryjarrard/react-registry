...
const HomePage = () => {
    ...
    const renderedPackages = featuredPackages.map((pack) => {
        return <div key={pack.name} className="flex flex-col justify-between gap-3 border rounded shadow p-4">
            ...
            <Link to={`/packages/${pack.name}`} className="border rounded border-gray-900 text-center">View Details</Link>
        </div>;
    });
        ...
        <div className="container py-12 space-y-8">
            <div className="space-y-6 text-center">
                ...
                <p className="mx-auto max-w-[600px] text-gray-500">
                    Welcome to the NPM Registry! Here you can find all the packages you need to build your next project.
                </p>
            </div>
            <div className="mx-auto grid grid-cols-4 max-w-[900px] items-stretch gap-4 text-center">
                {renderedPackages}
            </div>
        </div>
    );
